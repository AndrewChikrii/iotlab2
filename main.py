...
def read_processed_agent_data(processed_agent_data_id: int):
    with SessionLocal() as session:
        query = session.query(ProcessedAgentData).filter(ProcessedAgentData.id == processed_agent_data_id).first()
        if not query:
...
def list_processed_agent_data():
    with SessionLocal() as session:
        query = session.query(ProcessedAgentData).all()
        return query
...
def update_processed_agent_data(processed_agent_data_id: int, data: ProcessedAgentData):
    ...
    with SessionLocal() as session, session.begin():
        query = (
            session.query(ProcessedAgentData)
            .filter(ProcessedAgentData.id == processed_agent_data_id)
            .update({
                ProcessedAgentData.road_state: data.road_state,
                ProcessedAgentData.user_id: agent_data.user_id,
                ProcessedAgentData.x: agent_data.accelerometer.x,
                ProcessedAgentData.y: agent_data.accelerometer.y,
                ProcessedAgentData.z: agent_data.accelerometer.z,
                ProcessedAgentData.latitude: agent_data.gps.latitude,
                ProcessedAgentData.longitude: agent_data.gps.longitude,
                ProcessedAgentData.timestamp: agent_data.timestamp,
            }, returning=True)
        )
